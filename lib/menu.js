...
const ROOT_KEYBOARD = Markup.inlineKeyboard([
  [Markup.button.callback("⬇️ Download", "menu:download"), Markup.button.callback("🔄 Convert", "menu:convert")],
  [Markup.button.callback("⚙️ Pengaturan", "menu:settings"), Markup.button.callback("📜 Riwayat", "menu:history")],
  [Markup.button.callback("📖 Semua Perintah", "menu:help")],
]);

function backKeyboard() {
  return Markup.inlineKeyboard([[Markup.button.callback("⬅️ Kembali", "menu:root")]]);
}
...
function downloadCaption() {
  return (
    ...
    "<code>/formats &lt;link&gt;</code> — lihat semua format mentah kalau 360/720/1080p gak cocok"
  );
}
...
function convertCaption() {
  return (
    ...
    "⚠️ <b>Karena batas Telegram Bot API, file yang bisa diambil bot maksimal ±20MB.</b>"
  );
}
...
function helpCaption(maxFileSizeMB) {
  return (
    ...
    "• Gagal download? Ada tombol \"🔄 Coba lagi\" di pesan errornya, gak perlu kirim ulang link\n" +
    "• File hasil download kegedean → otomatis dikompres dulu sebelum nyerah\n\n" +
    "<b>Command</b>\n" +
    "<code>/menu</code> — buka menu utama\n" +
    "<code>/help</code> — pesan ini\n" +
    "<code>/setquality</code> — atur kualitas video default\n" +
    "<code>/autodelete</code> — atur auto-hapus pesan hasil download\n" +
    "<code>/formats &lt;link&gt;</code> — lihat semua format mentah dari yt-dlp (buat kasus edge)\n" +
    "<code>/cancel</code> — batalin download yang lagi jalan/antri\n" +
    "<code>/history</code> — riwayat download terakhir\n" +
    "<code>/cookiecheck</code> — tes manual kesehatan cookies Instagram\n" +
    "<code>/status</code> — info bot &amp; server (uptime, RAM/CPU/disk, antrian, versi yt-dlp)\n" +
    "<code>/stats</code> — statistik total download sepanjang waktu\n" +
    "<code>/ping</code> — cek bot masih hidup + latency\n\n" +
    `⚠️ <b>Batas ukuran file kirim: ${maxFileSizeMB}MB</b> (limit Telegram Bot API)`
  );
}
